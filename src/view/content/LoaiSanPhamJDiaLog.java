...
public class LoaiSanPhamJDiaLog extends javax.swing.JDialog {
    ...
    private void filltotableLoaiVi() {
        ...
        for (LoaiSanPham x : list) {
            model.addRow(new Object[]{x.getMaLoaiSanPham(), x.getTenLoaiSanPham(), x.isTrangThai() ? "Còn Hàng" : "Hết Hàng"});
        }

    }

    private LoaiSanPham getInformation() {
        LoaiSanPham sp = new LoaiSanPham();
        sp.setMaLoaiSanPham(txtMa.getText());
        sp.setTenLoaiSanPham(txtTen.getText());
        sp.setTrangThai(jRadioButton1.isSelected());
        return sp;
    }

    private void setForm(LoaiSanPham sp) {
        txtMa.setText(sp.getMaLoaiSanPham());
        txtTen.setText(sp.getTenLoaiSanPham());
    ...
    private void edit() {
        ...
        LoaiSanPham lsp = dao.selectID1(idloaisp);
        setForm(lsp);
    }
    ...
    private void insert() {
        if (Checknull()) {
            ...
            try {
                LoaiSanPham lsp = getInformation();
                dao.insert(lsp);
                filltotableLoaiVi();
                lammoi();
    ...
    private boolean Checknull() {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (id.equalsIgnoreCase(list.get(i).getMaLoaiSanPham())) {
                JOptionPane.showMessageDialog(this, "Trùng khóa chính");
                txtMa.requestFocus();
                return true;
            } else if (tensp.equalsIgnoreCase(list.get(i).getTenLoaiSanPham())) {
